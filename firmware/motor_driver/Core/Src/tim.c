...
void _writeDutyCycle3PWM(BLDC_PWM *bldc_pwm, float duty_u, float duty_v, float duty_w)
{
  ...
  __HAL_TIM_SET_COMPARE(bldc_pwm->htim, bldc_pwm->Channel_U, (PWM_PERIOD_CYCLES - 1) * duty_u);
  __HAL_TIM_SET_COMPARE(bldc_pwm->htim, bldc_pwm->Channel_V, (PWM_PERIOD_CYCLES - 1) * duty_v);
  __HAL_TIM_SET_COMPARE(bldc_pwm->htim, bldc_pwm->Channel_W, (PWM_PERIOD_CYCLES - 1) * duty_w);
}
/* USER CODE END 1 */
